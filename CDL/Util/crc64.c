 ...
 *  @author   alcoco
 *  @date     
 *   Created:       13:32:15 05/04/2005
 *   Last Update:   01:16:23 06/04/2005
 */
//========================================================================
#include "crc64.h"
...
void CRC64Reset(CRC64Context *crc)
{
    crc->crc=0xffffffffffffffffLL; 
}

void CRC64Input(CRC64Context *crc, const void *data, const int length)
{
    int len=length;
    unsigned long long crc0=crc->crc;
...
void CRC64Result(CRC64Context *crc, void *res)
{
    int i;
    crc->crc^=0xffffffffffffffffLL; 
    for(i=0; i<8; i++)
        ((unsigned char *)res)[i]=(crc->crc>>(i*8))&0xFF;
}
